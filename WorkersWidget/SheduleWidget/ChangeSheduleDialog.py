...
class ChangeSheduleDialog(QtWidgets.QDialog):

    def __init__(self, listOfFilms, listOfHalls, previousDate, previousFilm, previousHall, *args, **kwargs):
        super(ChangeSheduleDialog, self).__init__(*args, **kwargs)
        ...
        self.date.setSelectedDate(previousDate)
        ...
        for i in range(len(listOfHalls)):
            ...
            if previousHall == listOfHalls[i][0]:
                self.listOfHalls.setCurrentText(listOfHalls[i][1])
